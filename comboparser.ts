...
export type ParserStateHandler<OutResult = any, OutError = any, InResult = any> =
    (state: ParserState<InResult>) => ParserOutput<OutResult, OutError>;
export type ParserErrorHandler<OutError = any, InError = any> =
    (state: ParserError<InError>) => ParserError<OutError>;
export type ParserHandler<
    OutResult = any, OutError = any,
    InResult = any, InError = any
> = (state: ParserOutput<InResult, InError>) => ParserOutput<OutResult, OutError>;
export type SourceParser<Result = any, Error = any> = (source: string) => ParserOutput<Result, Error>;

export type Parser<
    OutResult = any, OutError = any,
    InResult = any, InError = any
> = ParserHandler<OutResult, OutError, InResult, InError> & SourceParser<OutResult, OutError> & {
...
export type BinaryParserOutput<Result = any, Error = any> = BinaryParserState<Result> | BinaryParserError<Result, Error>;

export type BinaryParserStateHandler<OutResult = any, OutError = any, InResult = any> =
    (state: BinaryParserState<InResult>) => BinaryParserOutput<OutResult, OutError>;
export type BinaryParserErrorHandler<OutError = any, InError = any> =
    (state: BinaryParserError<InError>) => BinaryParserError<OutError>;
export type BinaryParserHandler<
    OutResult = any, OutError = any,
    InResult = any, InError = any
> = (state: BinaryParserOutput<InResult, InError>) => BinaryParserOutput<OutResult, OutError>;
export type BinarySourceParser<Result = any, Error = any> = (source: string) => BinaryParserOutput<Result, Error>;

export type BinaryParser<
    OutResult = any, OutError = any,
    InResult = any, InError = any
> = BinaryParserHandler<OutResult, OutError, InResult, InError> & BinarySourceParser<OutResult, OutError> & {
    /** Wether the parse operates at binary level */
     ...
     */
    chain<OR, OE>(callback: BinMap<OutResult, BinaryParser<OR, OE, OutResult, OutError>>): BinaryParser<OR, OE, OutResult, OutError>;
};

export namespace state {
    export function ok<Input, Output>(state: ParserState<Input>, result: Output, index?: number): ParserState<Output>;
    export function ok<Input, Output>(state: BinaryParserState<Input>, result: Output, index?: number): BinaryParserState<Output>
    export function ok<Input, Output>(state: BinaryParserState<Input> | ParserState<Input>, result: Output, index?: number): BinaryParserState<Output> | ParserState<Output> {
        index ||= state.index;
    ...
    export function error<Input, Output>(state: BinaryParserOutput<Input, any>, error: Output): BinaryParserError<Input, Output>;
    export function error<Input, Output>(state: BinaryParserOutput<Input, any> | ParserOutput<Input, any>, error: Output): BinaryParserError<Input, Output> | ParserError<Input, Output> {
...
export function parser<OutResult = any, OutError = any, InResult = any, InError = any>(
    ...
    const out: Parser<OutResult, OutError, InResult, InError> = typeof(error) === 'function' ?
        ((state: string | ParserOutput) => 
            typeof(state) === 'string' ?
            handler({ source: state, index: 0, result: null as any, isError: false, error: null }) :
            state.isError ? error(state) : handler(state)
        ) : ((state: string | ParserOutput) =>
            typeof(state) === 'string' ?
                handler({ source: state, index: 0, result: null as any, isError: false, error: null }) :
            state.isError ? state : handler(state)
        ) as any;
    out.map = (fn: Map<OutResult>) => _parser(s => {
        const next = out(s);
        return next.isError ? next : _state(next, fn(next.result, next.index, next.source));
    });
    out.error = (fn: Map<OutError>) => _parser(s => {
        const next = out(s);
        return next.isError ? _error(next, fn(next.error, next.index, next.source)) : s as any;
    });
    out.chain = (fn: Map<OutResult, Parser<any, any, OutResult, OutError>>) => _parser(s => {
...
function _toBinary(value: Binariable): DataView {
    ...
    if (typeof(value) === 'string') { value = value.split('').map(s => s.charCodeAt(0)); }
    if (Array.isArray(value)) { value = new Uint8ClampedArray(value); }
...
export function binary<OutResult = any, OutError = any, InResult = any, InError = any>(
    ...
    const out: BinaryParser<OutResult, OutError, InResult, InError> = typeof(error) === 'function' ?
    ...
    out.map = (fn: BinMap<OutResult>) => _binary(s => {
        const next = out(s);
        return next.isError ? next : _state(next, fn(next.result, next.index, next.source));
    });
    out.error = (fn: BinMap<OutError>) => _binary(s => {
        const next = out(s);
        return next.isError ? _error(next, fn(next.error, next.index, next.source)) : s as any;
    });
    out.chain = (fn: BinMap<OutResult, BinaryParser<any, any, OutResult, OutError>>) => _binary(s => {
        const next = out(s as any);
        return next.isError ? next : fn(next.result, next.index, next.source)(next);
    });
...
const _match_string_ic = (expression: string, state: ParserState) =>
    ...
    _error(state, `Tried to match '${expression}', but got '${state.source.slice(state.index, state.index + expression.length)}' @ index ${state.index}`)
const _match_string_nc = (expression: string, state: ParserState) =>
    state.source.slice(state.index, state.index + expression.length) === expression ?
    _state(state, expression, state.index + expression.length) :
    _error(state, `Tried to match '${expression}', but got '${state.source.slice(state.index, state.index + expression.length)}' @ index ${state.index}`)
const _match_regex_raw = (expression: RegExp, state: ParserState) => {
    const match = state.source.slice(state.index).match(expression);
    return match ?
            _state(state, match[0]!, state.index + match[0]!.length) :
            _error(state, `Tried to match '${expression}', but got '${state.source.slice(state.index, state.index + 20)}' @ index ${state.index}`);
};
const _match_regex_map = (expression: RegExp, map: (match: RegExpMatchArray) => any, state: ParserState) => {
    const match = state.source.slice(state.index).match(expression);
    return match ?
...
export function match(expression: string | RegExp, matcher?: boolean | ((match: RegExpMatchArray) => any)) {
    if (expression instanceof RegExp) {
        expression.source[0] !== '^' && (expression = new RegExp(
            '^' + expression.source,
    ...
    return _parser<string, string>(
        expression instanceof RegExp ?
            typeof(matcher) === 'function' ? _match_regex_map.bind(null, expression, matcher) : _match_regex_raw.bind(null, expression) :
            matcher ? _match_string_ic.bind(null, expression.toLowerCase()) : _match_string_nc.bind(null, expression)
    );
}
...
export const eof = _parser(state => 
    ...
    _error(state, `Expected End Of Input (eoi/eof/end) but found '${state.source[state.index]}' @ index ${state.index}`)
);
/** A parser which check the end of the input */
...
type SequenceOfParsersResult<Parsers extends Parser[]> =
    Parsers extends [] ? [] :
    Parsers extends [ Parser<infer OR> ] ? [ OR ] :
    Parsers extends [ Parser<infer OR1, infer OE1>, ...infer Rest1 ] ?
        Rest1 extends [ Parser<infer OR2, any, OR1, OE1>, ...infer Rest2 ] ? Rest2 extends Parser[] ?
            [ OR1, OR2, ...SequenceOfParsersResult<Rest2> ] :
        never : never :
    Parser extends Parser<infer OR>[] ? OR[] :
    never;
type SequenceOfParsersError<Parsers extends Parser[]> =
    Parsers extends [] ? never :
    Parsers extends [ Parser<any, infer OE> ] ? OE :
    Parsers extends [ Parser<infer OR1, infer OE1>, ...infer Rest1 ] ?
        Rest1 extends [ Parser<any, infer OE2, OR1, OE1>, ...infer Rest2 ] ? Rest2 extends Parser[] ?
            OE1 | OE2 | SequenceOfParsersError<Rest2> :
        never : never :
    Parser extends Parser<any, infer OE>[] ? OE :
    never
type SequenceOfParsers<Parsers extends Parser[]> = 
    SequenceOfParsersResult<Parsers> extends never ? never :
    SequenceOfParsersError<Parsers> extends never ? never :
...
export function sequence(...parsers: Parser[]): any {
    ...
    if (!parsers.length) { throw new TypeError('sequence: you must provide at least one parser'); }
...
type OneOf<Args extends any[]> =
    Args extends [] ? never :
    Args extends [ infer X ] ? X :
    Args extends [ infer X, ...infer Rest ] ? X | OneOf<Rest> :
    Args extends (infer X)[] ? X : 
    never;

type ParserOneOf<Parsers extends Parser[]> =
    SequenceOfParsersResult<Parsers> extends never ? never :
    SequenceOfParsersError<Parsers> extends never ? never :
...
export function oneOf(...parsers: Parser[]): any {
    parsers = _extract(parsers);
    if (!parsers.length) { throw new TypeError('oneOf: you must provide at least one parser'); }
    return _parser(state => {
...
export function many<P extends Parser>(parser: P): ManyOf<P> {
    if (typeof(parser) !== 'function') { throw new TypeError('many: parser must be a valid parser function'); }
    return _parser(state => {
        let next: ParserOutput = state;
        const results = [];
        while (true) {
...
export function oneOrMore<P extends Parser>(parser: P): ManyOf<P> {
    if (typeof(parser) !== 'function') { throw new TypeError('oneOrMore: parser must be a valid parser function'); }
    return _parser(state => {
        let next: ParserOutput = state;
        const results = [];
        while (true) {
...
export function times<P extends Parser>(parser: P, times: number): ManyOf<P> {
    if (typeof(parser) !== 'function') { throw new TypeError('times: parser must be a valid parser function'); }
    times = Number(times);
    if (isNaN(times)) { throw new TypeError('times: times argument must be a valid number'); }
    if (times < 1) { throw new RangeError(`times: times argument must be at least 1, ${times} given`)}
    return _parser(state => {
        let next: ParserOutput = state;
        const results = [];
        for (let i = 0; i < times; ++i) {
            next = parser(next);
            if (next.isError) { return _error(state, `times: Unable to match the input parser ${times} times @ index ${state.index}`); }
...
export function between<
    OR1 = any, OE1 = any, IR1 = any, IE1 = any,
    OR2 = any, OE2 = any, OR3 = any, OE3 = any
>(
 ...
 */
export function parseBetween<OR = any, OE = any, IR = any, IE = any>(wrapper: Parser<OR, OE, IR, IE>): (content: Parser<IR, IE, OR, OE>) => Parser<OR, OE, IR, IE>;
/**
 * Generate an high-order function which creates a
 * between expression for a parser. This function 
 * returns another function which takes the content
...
export function parseBetween(left: Parser, right?: Parser) { return (content: Parser) => between(left, content, right!); }
 ...
 * @return an high-order function which generate a between parser expression
 */
export function parseSeparated<OR1, OE1, IR1, IE1>(separator: Parser<OR1, OE1, IR1, IE1>) :
...
export function lazy<P extends Parser>(thunk: () => P, notCached?: boolean): P {
    if (notCached) { return _parser(state => thunk()(state)) as P; }
    let res: P | null = null;
 ...
 */
export function fail<E>(message: E): Parser<any, E> { return _parser(state => _error(state, message)); }
/**
 ...
 */
export function success<T>(value: T): Parser<T> { return _parser(state => _state(state, value)); }

type ContextualOneOf<P extends Parser> =
    P extends Parser<infer R> ? R :
    P extends Parser<infer R> | infer Rest ?
        Rest extends Parser ?
        ContextualOneOf<Rest> extends never ? never :
...
export function contextual<P extends Parser = Parser, R = any>(generator: ContextualCB<P, R>): Parser<R> {
    return success(null).chain(() => {
        ...
        function runStep(next: ContextualOneOf<P>): Parser {
            ...
            if (itResult.done) { return success(itResult.value); }
            const nextParser = itResult.value;
            if (typeof(nextParser) !== 'function') { throw new TypeError('contextual: yielded values must always be parsers!'); }
...
export namespace binary {
    ...
    function _compare(
        ...
        if (leftLength - leftOffset < maxOffset) { return false; }
     ...
     */
    export function match(expression: Binariable, expressionOffset: number = 0, expressionLength: number = -1): BinaryParser<DataView, string> {
        expression = _toBinary(expression);
        return _binary(state =>
            _compare(
                state.source, expression,
                state.index, expressionOffset, -1,
                expressionLength
            ) ?
            _state(state, expression, state.index + (expression as DataView).byteLength) :
            _error(state, `Tried to match '${_str(expression)}', but got '${_str(state.source, state.index, state.index + _len(expression))}' @ index ${state.index}`)
        );
    }
    ...
    export const bit: BinaryParser<0 | 1, string> = _binary(state => {
        ...
        if (byteOffset >= state.source.byteLength) { return _error(state, `bit: Unexpected end of input @ index ${state.index}`); }
    ...
    export function uint(bits: number, bigEndian?: boolean): BinaryParser<number, string> {
        if (bits < 1) { throw new RangeError(`uint: btis must be larger than 0, got ${bits}`)}
        if (bits > 32) { throw new RangeError(`uint: btis must be less or equal to 32, got ${bits}`)}
        const seq = [];
        for (let i = 0; i < bits; ++i) { seq.push(bit); }
    ...
    export function biguint(bits: number, bigEndian?: boolean): BinaryParser<bigint, string> {
        if (bits < 1) { throw new RangeError(`biguint: btis must be larger than 0, got ${bits}`)}
        const seq = [];
        for (let i = 0; i < bits; ++i) { seq.push(bit); }
    ...
    export function int(bits: number, bigEndian?: boolean): BinaryParser<number, string> {
        if (bits < 1) { throw new RangeError(`int: btis must be larger than 0, got ${bits}`)}
        if (bits > 32) { throw new RangeError(`int: btis must be less or equal to 32, got ${bits}`)}
        const seq = [];
        for (let i = 0; i < bits; ++i) { seq.push(bit); }
        return sequence(seq).map(
            bigEndian ?
    ...
    export function bigint(bits: number, bigEndian?: boolean): BinaryParser<bigint, string> {
        if (bits < 1) { throw new RangeError(`bigint: btis must be larger than 0, got ${bits}`)}
        const seq = [];
        for (let i = 0; i < bits; ++i) { seq.push(bit); }
        const _biBits = BigInt(bits - 1);
        return sequence(seq).map(
    ...
    type SequenceOfParsersResult<Parsers extends BinaryParser[]> =
        Parsers extends [] ? [] :
        Parsers extends [ BinaryParser<infer OR> ] ? [ OR ] :
        Parsers extends [ BinaryParser<infer OR1, infer OE1>, ...infer Rest1 ] ?
            Rest1 extends [ BinaryParser<infer OR2, any, OR1, OE1>, ...infer Rest2 ] ? Rest2 extends BinaryParser[] ?
                [ OR1, OR2, ...SequenceOfParsersResult<Rest2> ] :
            never : never :
        Parsers extends BinaryParser<infer OR>[] ? OR[] :
        never;
    type SequenceOfParsersError<Parsers extends BinaryParser[]> =
        Parsers extends [] ? never :
        Parsers extends [ BinaryParser<any, infer OE> ] ? OE :
        Parsers extends [ BinaryParser<infer OR1, infer OE1>, ...infer Rest1 ] ?
            Rest1 extends [ BinaryParser<any, infer OE2, OR1, OE1>, ...infer Rest2 ] ? Rest2 extends BinaryParser[] ?
                OE1 | OE2 | SequenceOfParsersError<Rest2> :
            never : never :
        Parsers extends BinaryParser<any, infer OE>[] ? OE :
        never
    type SequenceOfParsers<Parsers extends BinaryParser[]> = 
        SequenceOfParsersResult<Parsers> extends never ? never :
        SequenceOfParsersError<Parsers> extends never ? never :
    ...
    type ParserOneOf<Parsers extends BinaryParser[]> =
        SequenceOfParsersResult<Parsers> extends never ? never :
        SequenceOfParsersError<Parsers> extends never ? never :
        Parser<OneOf<SequenceOfParsersResult<Parsers>>, SequenceOfParsersError<Parsers>, any, any>
    ...
    export function oneOf(...parsers: BinaryParser[]): ParserOneOf<BinaryParser[]> {
        ...
        return _binary(state => {
            ...
            return _error(
                state,
                `oneOf: Unable to match with any parser @ index ${state.index}`
            );
        }) as any;
    }

    type ManyOf<P> = P extends BinaryParser<infer OR, infer OE, infer IR, infer IE> ? BinaryParser<OR[], OE, IR, IE> : never;
     ...
     */
    export function parseBetween<OR = any, OE = any, IR = any, IE = any>(wrapper: BinaryParser<OR, OE, IR, IE>): (content: BinaryParser<IR, IE, OR, OE>) => BinaryParser<OR, OE, IR, IE>;
    /**
     * Generate an high-order function which creates a
     * between expression for a binary parser. This function 
     * returns another function which takes the content
    ...
    export function parseBetween(left: BinaryParser, right?: BinaryParser) { return (content: BinaryParser) => between(left, content, right!); }
     ...
     * @return an high-order function which generate a binary between parser expression
     */
    export function parseSeparated<OR1, OE1, IR1, IE1>(separator: BinaryParser<OR1, OE1, IR1, IE1>) :
    ...
    export function lazy<P extends BinaryParser>(thunk: () => P, notCached?: boolean): P {
        if (notCached) { return _binary(state => thunk()(state)) as P; }
        let res: P | null = null;
     ...
     */
    export function fail<E>(message: E): BinaryParser<any, E> { return _binary(state => _error(state, message)); }
    /**
     * Create a binary parser which always produces a success binary parser state
     * @param value the result value of the success state
     * @returns a success binary parser
     */
    export function success<T>(value: T): BinaryParser<T> { return _binary(state => _state(state, value)); }

    type ContextualOneOf<P extends BinaryParser> =
        P extends BinaryParser<infer R> ? R :
     ...
     */
    export function contextual<P extends BinaryParser = BinaryParser, R = any>(generator: ContextualCB<P, R>): BinaryParser<R> {
        return success(null).chain(() => {
            const iterator = generator();
    ...
    export function asBinary(value: string | (boolean | 1 | 0 | '1' | '0')[]): DataView {
        if (typeof(value) === 'string') { value = value.replace(_asBinaryRep, '').split('').map(v => v === '1'); }
        const output = [];
        const len = Math.ceil(value.length / 8);
        for (let i = 0; i < len; ++i) {
            output[i] = 0;
            for (let j = 0; j < 8; ++j) { output[i] |= (Number(value[i * 8 + j]) ? 1 : 0) << (7 - j); }
        }
